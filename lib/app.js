...
(function() {
  ...
  ksApp = (function() {
    ...
    function ksApp(ks) {
      var options, _base, _base1, _base2, _ref, _ref1, _ref2, _ref3, _ref4;
      ...
      if (this.config.newRelic.license_key) {
        if ((_ref = (_base = process.env).NEW_RELIC_NO_CONFIG_FILE) == null) {
          _base.NEW_RELIC_NO_CONFIG_FILE = true;
        }
        options = this.config.newRelic;
        if ((_ref1 = options.app_name) == null) {
          options.app_name = ["Tapas: " + this.config.locals.site_name];
        }
        if ((_ref2 = options.component) == null) {
          options.component = 'express_random_delays';
        }
        if ((_ref3 = (_base1 = process.env).NEW_RELIC_APP_NAME) == null) {
          _base1.NEW_RELIC_APP_NAME = options.app_name;
        }
        if ((_ref4 = (_base2 = process.env).NEW_RELIC_LICENSE_KEY) == null) {
          _base2.NEW_RELIC_LICENSE_KEY = options.license_key;
        }
        require('newrelic');
        this.logger = require('newrelic/lib/logger').child(options);
        this.logger.log = this.logger.trace;
      }
